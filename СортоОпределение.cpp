...
{
    fstream temp; const int* first; const int* second;
...
void merge_files()
{
    fstream res; fstream temp1; fstream temp2;
    ...
    if (res.is_open())
        res.close();
    if (! filesystem::copy_file (" temp_2_file.txt", "res_file.txt", filesystem::copy_options::overwrite_existing))
        return;
}
...
{
    arr = new int[PART_SIZE]; int* tmp_arr; int i; int j;
...
void write_rand_arr(const string& filename)
{
    fstream fs;
    srand(time(nullptr));
    int lef_border = -100; int range_len = 50000;
